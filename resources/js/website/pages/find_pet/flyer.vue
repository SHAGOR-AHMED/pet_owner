<template>
      ...
      <v-card class="space px-2 mb-10">
        <v-card-text>
          <div class="converttoimagebtn d-flex justify-content-center">
            <input id="btn_flyer" type="button" class="btn btn-success" value="Download Flyer" @click="downloadFlyer()"/>&nbsp;
            <a href="/do-logout" class="btn btn-danger text-white">
              Logout
            </a>&nbsp;
            <router-link :to="{ name: 'FindPet'}" class="btn btn-primary text-white">
              Back
            </router-link>
          </div>
          <div id="previewImg2"></div>
            <div id="find_flyer" style="width:720px;height:613px;margin:auto;padding-top:10px;padding-left:10px; position:relative;display:block;">
                <div class="flyerdesigner">
                    <div class="flyerheader">
                        <h1 class="flyertitle">Found Dog</h1>
                    </div>
                    <div class="flyerbody">
                        <div class="flyerpetfoundphoto">
                            <img :src="imagePathSm + detailsData.image" alt="Find Pet">
                        </div>
                        <div class="flyerpetdetails">
                            ...
                            <p class="whereitslost"><span class="flyerdtitle">Found Location: </span> {{ detailsData.found_location }}</p>
                        </div>
                    </div>
  ...
  export default {
      ...
      methods: {
        ...
        downloadFlyer(){
          html2canvas(document.getElementById("find_flyer"),
            {
                allowTaint: true,
                useCORS: true
                ...
                anchorTag.download = "find_flyer.jpg";
...
.flyerpetname{font-size:35px; text-transform:uppercase;color:#eb2411;margin:0;line-height: 53px;}
...
.rewardamount{text-align:center;color:#fff;font-size:69px;text-transform:uppercase;margin:0;line-height: 63px;}
.rewardtitle{text-align:center;color:#fff;font-size:40px;text-transform:uppercase;margin-bottom:5px;margin-top:20px;line-height: 30px;}
